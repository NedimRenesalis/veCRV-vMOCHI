...
import { increase, mine, getTimestamp, getBlockNumber, increaseTo } from "./utils";
...
describe('vMochi', function(){
  ...
  beforeEach(async function() {
    ...
    const MochiFactory = await ethers.getContractFactory("MockToken");
  ...
  describe("#createLock()", function(){
    ...
    it("sanity check", async function(){
      expect(await vmochi["balanceOf(address)"](alice.getAddress())).to.equal(0);
    });

    it("should fail if _value == 0", async function(){
      await expect(vmochi.connect(alice).createLock(0, now.add(WEEK))).to.be.revertedWith("zero value");
    });

    it("should fail if due is past", async function(){
      await expect(vmochi.connect(alice).createLock(MOCHI, now.sub(1))).to.be.revertedWith("Can only lock until time in the future");
    });

    it("should fail if dus is more than 4 years", async function(){
      await expect(vmochi.connect(alice).createLock(MOCHI, now.add(WEEK.mul(52*5)))).to.be.revertedWith("Voting lock can be 4 years max");
    });
    ...
    it("should fail if lock exists", async function(){
      ...
      await expect(vmochi.connect(alice).createLock(MOCHI, now.add(WEEK))).to.be.revertedWith("Withdraw old mochis first");
    });

    describe("sanity check", function(){
      const durations = [WEEK, WEEK.mul(2), WEEK.mul(10), WEEK.mul(52), WEEK.mul(104)];
      durations.forEach(d => {
  ...
  describe("#increaseAmount()", function(){
    ...
    it("should fail if _value == 0", async function(){
      await expect(vmochi.connect(alice).increaseAmount(0)).to.be.revertedWith("zero value");
    });

    it("should fail if lock does not exist", async function(){
      await expect(vmochi.connect(bob).increaseAmount(MOCHI)).to.be.revertedWith("No existing lock found");
    });
    ...
    it("should fail if lock exists", async function(){
      ...
      await mine();
      await mochi.connect(alice).approve(vmochi.address, MOCHI);
      await expect(vmochi.connect(alice).increaseAmount(MOCHI)).to.be.revertedWith("Cannot add to expired lock. Withdraw");
    });
    ...
    it("effect", async function(){
      ...
      const balance = await vmochi["balanceOf(address)"](alice.getAddress());
      almostEqual(balance, MOCHI.mul(2).div(await vmochi.MAXTIME()).mul(WEEK));
    });
  });
  describe("#increaseUnlockTime()", function(){
    ...
    it("should fail if lock does not exist", async function(){
      await expect(vmochi.connect(bob).increaseUnlockTime(now.add(WEEK.mul(2)))).to.be.revertedWith("No existing lock found");
    });
    ...
    it("should fail if lock exists", async function(){
      ...
      await mine();
      await mochi.connect(alice).approve(vmochi.address, MOCHI);
      await expect(vmochi.connect(alice).increaseUnlockTime(now.add(WEEK.mul(2)))).to.be.revertedWith("Lock expired");
    });
